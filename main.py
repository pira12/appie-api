...
def scrape_data():
    ...
    for shift in all_shifts:
        ...
        shift[1][0] = convert_to_RFC_datetime(year, month, int(shift[0]), int(shift[1][0].split(':')[0]) + hour_adjustment, int(shift[1][0].split(':')[1]))
        shift[1][1] = convert_to_RFC_datetime(year, month, int(shift[0]), int(shift[1][1].split(':')[0]) + hour_adjustment, int(shift[1][1].split(':')[1]))
        shift = shift[1:2]
...
def clear_calendar():
    ...
    events = service.events().list(calendarId=my_calender_id).execute().get('items')

    for event in events:
        service.events().delete(calendarId=my_calender_id, eventId=event.get('id')).execute()
